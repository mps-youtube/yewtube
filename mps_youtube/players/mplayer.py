...
class Mplayer(Player):
    ...
    def _playback_progress(self, idx, allsongs, repeat=False):
        ...
        show_key_help = config.SHOW_PLAYER_KEYS.get
        ...
        keys = super()._help(short=(not multi and not repeat))
    ...
    def _playsong(self, song, failcount=0, override=False):
        ...
        try:
            ...
            stream = streams.select(cached, q=failcount, audio=(not video), m4a_ok=False)
        ...
        cmd = self._generate_real_playerargs(song, override, stream, video)
    ...
    def _generate_real_playerargs(self, song, override, stream, isvideo):
        ...
        return ["mplayer"] + args + [stream['url']]
    ...
    def _launch_player(self, song, songdata, cmd):
        ...
        try:
            ...
            self._player_status(p, songdata + "; ", song.length)
            returncode = p.wait()
    ...
    def _player_status(self, po_obj, prefix, songlength=0, mpv=False, sockpath=None):
