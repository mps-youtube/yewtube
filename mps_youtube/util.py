...
def yt_datetime(yt_date_time):
    ...
    d = datetime.fromisoformat(yt_date_time[:-1])
    y = d.strftime('%Y-%m-%d %H:%M:%S')
    time_obj = time.strptime(str(y), "%Y-%m-%d %H:%M:%S")
...
def yt_datetime_local(yt_date_time):
    ...
    datetime_obj = datetime.fromisoformat(yt_date_time[:-1])
